...
const unitUsed = require('../models/unit_used');
...
const GET_WPRICE = async (req, res) => {
    ...
    if (unitUsed.WATERUNIT > minUnit) {
        ...
        console.log(unitUsed.WATERUNIT, " - ", minUnit, " * ", pricePerUnit, " + ", minPrice)
    } else {
        price = minPrice
    }
    console.log("price: ", price)
...
const ADD_UNIT_USED = async (req, res) => {
    ...
    eMeter.create(electricityMeter)
    .then(data => {
        res.send(data);
    })
    .catch(err => {
